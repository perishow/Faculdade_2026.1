...
def gerar_pdf_relatorio(
    ...
    mae_base,
    imagem_grafico,
    nome_arquivo_pdf="../relatorios/Relatorio_Modelos_Hibridos.pdf",
):
    pdf = FPDF()
    pdf.add_page()
    ...
    # Título
    pdf.cell(
        0, 10, "Relatório de Desempenho: Modelos Híbridos vs SARIMA", ln=True, align="C"
    )
    pdf.ln(10)
...
for nome, (mod, params) in modelos.items():
    ...
    grid = GridSearchCV(
        mod, params, cv=tscv, scoring="neg_mean_squared_error", n_jobs=-1
    )
...
for nome, mets in metricas_modelos.items():
    sinal = "+" if mets["Melhoria"] > 0 else ""
    print(
        f"{nome:<25} | MSE: {mets['MSE']:.2f} | MAE: {mets['MAE']:.2f} | Melhoria: {sinal}{mets['Melhoria']:.2f}%"
    )
...
def plotar_comparacao_total(real, sarimax, prevs_dict, amostras=150):
    ...
    plt.title(
        f"Comparação de Modelos Híbridos na Escala Real (Primeiras {amostras} horas)",
        fontsize=14,
    )
    ...
    nome_arquivo = (
        "../plotagens/plotagem_comparativa_hibrido/grafico_comparativo_modelos.png"
    )
